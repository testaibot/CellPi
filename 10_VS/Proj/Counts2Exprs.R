...
{
  min_l = max(2, (min(ncol(sce)/20, 5)))
  max_l = min(ncol(sce)/5, 100)
  
  norm_levels = sort(unique(round(seq(min_l, max_l, max(1, (max_l - min_l)/30)))))
  
  sce = tryCatch(computeSumFactors(sce, sizes = norm_levels, positive = F),
                 warning = function(w)
                 {
                   print("Using inverse model for normalization. Can be slow on cells_count>1000")
...
sce_norm = function(sce, do.filter = T, spike.genes = NULL)
{
  ...
  sce_norm_base = function(sce, spike.genes = NULL)
  {
    sce1 = get_sum_factors(sce)
    if(!is.null(sce1))
    {
      sce = sce1
    }else{
    ...
    {
      sce1 = tryCatch(computeSpikeFactors(sce, type = "ercc", general.use = FALSE), error = function(e) {NULL})
      if(!is.null(sce1))
      {
        sce = sce1
      }
  ...
  sce1 = sce_norm_base(sce, spike.genes = spike.genes)
  if(!is.null(sce1))
  {
    return(sce1)
  }else{
    return(sce_norm_base(mtx_to_sce(rel2abs(counts(sce)))))
  }
...
clusters_mixed = function(object)
{
  return(F)
}
