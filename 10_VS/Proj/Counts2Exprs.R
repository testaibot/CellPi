...
{
  pc_seq = c(100,90,80,70,60,50,40,30,20,15,10,8,6,5,4,3,2)
  init_pc = min(max(nrow(object@raw.data)/120, 30), 100)
  
  for(pc_use in pc_seq[pc_seq<=init_pc])
  {
    #print(pc_use)
...
seurat_analyse_mtx = function(object, regress, regress_cc, do.magic, do.cluster = T, k.param_factor = 10)
{
  raw = object
...
cluster_recursive = function(object, regress = F, regress_cc = NULL, do.magic = F)
{
  ...
  tab = sort_diag(table(get_orig_ident(object), get_ident(object)))
  print(tab)
  
  while(any(sapply(sapply(get_ident(object), toString), pick_last_char)!="f"))
  {
    object = seur_merge(split_all_ident(object, regress = regress, regress_cc = regress_cc, do.magic = do.magic))
    tab = sort_diag(table(get_orig_ident(object), get_ident(object)))
    print(tab)
  }
