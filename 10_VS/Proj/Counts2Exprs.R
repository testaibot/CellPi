...
magic_impute <- function (mtx, components = 100)
{
  ...
  system(paste0('bash -c "MAGIC.py -d ', file_in, ' -o ', file_out, '  --cell-axis \'columns\' -n -p ', min(components, nrow(mtx)),' csv"'))
  
  magic_imputed <- read.csv(file = file_out)
  
  magic_imputed <- rel2abs(magicimpute2matrix(magic_imputed))
...
compute_clustering_min <- function(object, reduction.type = "pca", gran_shresh = 2, strict_binary = T)
{
  ...
  object <- FindClusters(object = object, reduction.type = reduction.type, algorithm = 3, k.param = k.param, k.scale = k.scale, modularity.fxn = 1, print.output = F, save.SNN = T, force.recalc = T)
...
seurat_analyse_mtx <- function(mtx, regress, regress_cc, do.magic, do.cluster = T)
{
  ...
  return(seurat_analyse(object, regress = regress, regress_cc = regress_cc, do.cluster = do.cluster))
}

seurat_analyse <- function(object, regress, regress_cc, do.cluster = T)
{
...
split_ident <- function(object, ident = NULL, gran_shresh = 2, regress, regress_cc, do.magic)
{
...
split_all_ident <- function(object, gran_shresh = 2, regress, regress_cc, do.magic)
{
  parallel_params_list <- lapply(unique(get_ident(object)), function(ident){ seur_get_raw_and_params(object, ident = ident, gran_shresh = gran_shresh, regress = regress, regress_cc= regress_cc, do.magic = do.magic)})
...
seur_merge <- function(list_seur)
{
  ...
  temp_object <- seurat_analyse(temp_object, regress = F, regress_cc = F, do.cluster = F)
