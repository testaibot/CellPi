...
{
  pc_seq = c(20, 10, 7, 6, 5, 4, 3, 2)
  
  for(pc_use in pc_seq)
  {
    object1 = tryCatch(RunPCA(object = object, pc.genes = rownames(object@raw.data), print.results = FALSE, pcs.print = NULL, do.print = F, pcs.compute = pc_use, rev.pca = F, weight.by.var = F),
                       error = function(e) {"e"}, 
...
{
  init_plex = ncol(object@raw.data)/3
  end_plex = 2
...
compute_clustering_min = function(object, strict_binary = T, k.param_factor = 2)
{
  ...
  cat("opt_pc:", length(good_PC))
  
  print("Reducing dims...")
  object = compute_tsne(object, 
                        dim.embed = min(5, length(good_PC)), pc.use = good_PC)
...
seurat_analyse_mtx = function(object, regress, regress_cc, do.magic, do.cluster = T, k.param_factor = 2)
{
  raw = object
  ...
  return(seurat_analyse(object, regress = regress, regress_cc = regress_cc, do.cluster = do.cluster, raw_before_magic = raw, k.param_factor = k.param_factor))
}

seurat_analyse = function(object, regress, regress_cc, do.cluster = T, raw_before_magic = NULL, k.param_factor = 2)
{
  ...
  if(do.cluster)
  {
    object1 = compute_clustering_min(object, k.param_factor = k.param_factor)
    if(!is.null(object1))
    {
      object = object1
      rm(object1)
...
seur_get_raw_and_params = function(object, ident = NULL, regress, regress_cc, do.magic)
{
  ...
  return(list(object = raw_data, ident = ident, regress = regress, regress_cc = regress_cc, do.magic = do.magic))
}
...
seur_find_split = function(object)
{
  ...
  return(list(obj = object, mark = NULL))
}
...
split_ident = function(object, ident = NULL, regress = F, regress_cc = NULL, do.magic = F)
{
  param_list_for_ident = seur_get_raw_and_params(object, ident = ident, regress = regress, regress_cc = regress_cc, do.magic = do.magic)
  return(seur_find_split(param_list_for_ident))
}

split_all_ident = function(object, regress = F, regress_cc = NULL, do.magic = F)
{
  parallel_list = lapply(unique(get_ident(object)), function(ident){ seur_get_raw_and_params(object, ident = ident, regress = regress, regress_cc =  regress_cc, do.magic = do.magic)})
...
{
  cl_idents = lapply(1:length(list_seur), function(i) {return(GetClusters(list_seur[[i]]$obj))})
  new_idents = cl_idents[[1]]
  merged_object = list_seur[[1]]$obj
  mark = list_seur[[1]]$mark
  
  meta = list(item = plot_seur(list_seur[[1]]$obj))
  ...
  for(i in 2:length(cl_idents))
  {
    ...
    merged_object = MergeSeurat(merged_object, list_seur[[i]]$obj, do.normalize = F)
    mark = list(mark, list_seur[[i]]$mark)
    
    meta = c(meta, list(item = plot_seur(list_seur[[i]]$obj)))
  ...
  return (list(obj = merged_object, mark = mark, meta = meta))
}
...
cluster_recursive = function(object, regress = F, regress_cc = NULL, do.magic = F, lvls = 5)
{
  #prevent non-parallel on fist split
  object = split_ident(CreateSeuratObject(raw.data = object), regress = regress, regress_cc = regress_cc, do.magic = do.magic)
  tab = sort_diag(table(get_orig_ident(object$obj), get_ident(object$obj)))
  print(tab)
  
  final_markers = rbind(NULL, object$mark)
  #meta = list(item = plot_seur(object$obj))
  
  while(any(sapply(sapply(get_ident(object$obj), toString), pick_last_char)!="f") & lvls>1)
  {
    lvls = lvls-1
    object = seur_merge(split_all_ident(object$obj, regress = regress, regress_cc = regress_cc, do.magic = do.magic))
    final_markers = rbind(final_markers, object$mark)
    #meta = c(meta, object$meta)
