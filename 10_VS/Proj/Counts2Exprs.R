...
find_cell_cycle_genes <- function(object)
{
  ...
  G2M <- gene_annot_convert_to(unique(cc$G2M$first), "ENSEMBL", org = genome$org, annot = genome$annot)$new
  S <- gene_annot_convert_to(unique(cc$S$first), "ENSEMBL", org = genome$org, annot = genome$annot)$new
  object1 <- tryCatch(CellCycleScoring(object, G2M, S, set.ident = F), error = function(e){NULL})
  return(object1)
}

seur_norm_regress <- function(object, regress = F, regress_cc_diff = F)
{
  ...
  if(regress)
  {
    object1 <- find_cell_cycle_genes(object)
    ...
    if(!is.null(object1))
    {
      ...
      if(regress_cc_diff)
      {
        ...
        object <- ScaleData(object = object, vars.to.regress = c("nUMI", "CC.Difference"), display.progress = T)
      }
      else{
        object <- ScaleData(object = object, vars.to.regress = c("nUMI", "S.Score", "G2M.Score"), display.progress = T)
      }
    }else{
      object <- ScaleData(object = object, vars.to.regress = c("nUMI"), display.progress = T)
    }
  }else{
...
compute_clustering_min <- function(object, gran_shresh = 2, strict_binary = T)
{
  ...
  {
    for(rez in seq(from + direction*(10^-depth), to, direction*(10^-depth)))
    {
      print(rez)
      test_rez <- test_clustering_rez(object, rez)
      print(test_rez)
      if(direction > 0)
      {
        if(is.null(test_rez)){
  ...
  object <- FindClusters(object = object, resolution = good_rez, k.param = k.param, k.scale = k.scale, algorithm = 3, modularity.fxn = 1, print.output = F, save.SNN = T, force.recalc = T)
  return(object)
}
...
seurat_analyse_mtx <- function(mtx, regress = F, regress_cc_diff = F, do.magic = F, do.cluster = T)
{
  if(do.magic)
  {
...
seurat_analyse <- function(object, regress = F, regress_cc_diff = F, do.cluster = T)
{
  object <- seur_norm_regress(object, regress = regress, regress_cc_diff = regress_cc_diff)
...
seur_get_raw_and_params <- function(object, ident = NULL, gran_shresh = 2, regress = F, regress_cc_diff = F, do.magic = F)
{
  ...
  return(list(object = raw_data, ident = ident, gran_shresh = gran_shresh, regress = regress, regress_cc_diff = regress_cc_diff, do.magic = do.magic))
}
...
seur_find_split <- function(object)
{
  ...
  if((!is.null(ident) && pick_last_char(toString(ident)) != "f") || is.null(ident) || ncol(raw_data)>=gran_shresh*2)
  {
    for(mult in seq(curr_mult, 10, 2))
    {
      ...
      object <- tryCatch(seurat_analyse_mtx(dub(raw_data, mult), regress = regress, regress_cc_diff = regress_cc_diff, do.magic = do.magic, do.cluster = T),error = function(e) {"e"})
  ...
  ending_clust <- CreateSeuratObject(raw.data = as(raw_data, "dgCMatrix"))
  
  cl_labels <- GetClusters(ending_clust)
  ...
  ending_clust <- SetClusters(ending_clust, cl_labels)
...
split_ident <- function(object, ident = NULL, gran_shresh = 2, regress = F, regress_cc_diff = F, do.magic = F)
{
  param_list_for_ident <- seur_get_raw_and_params(object, ident = ident, gran_shresh = gran_shresh, regress = regress, regress_cc_diff = regress_cc_diff, do.magic = do.magic)
...
split_all_ident <- function(object, gran_shresh = 2, regress = F, regress_cc_diff = F, do.magic = F)
{
  ...
  object_list <- parLapply(cl, parallel_params_list, function(param_list_for_ident){
    ...
    output <- seur_find_split(param_list_for_ident)
    return(output)
  })
  stopCluster(cl)
...
seur_merge <- function(list_seur)
{
  ...
  for(i in 2:length(cl_idents))
  {
    ...
    temp_object <- MergeSeurat(temp_object, list_seur[[i]])
  }
  temp_object <- SetClusters(temp_object, new_idents)
  temp_object <- seurat_analyse(temp_object, do.cluster = F)
