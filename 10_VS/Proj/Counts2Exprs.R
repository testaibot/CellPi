...
compute_clustering_min = function(object, strict_binary = T, k.param_factor = 2)
{
  ...
  print("Building SNN...")
  object = compute_tsne(object, dim.embed = 5)
  ...
  for(depth in 0:max_num)
  {
    ...
    for(rez in seq(from, to, direction*(10^-depth)))
    {
      ...
      print(table(get_orig_ident(object), get_ident(object)))
    }
...
seur_merge = function(list_seur)
{
  ...
  for(i in 2:length(cl_idents))
  {
    ...
    mark = rbind(mark, list_seur[[i]]$mark)
    list_seur[[i]] = "NULL"
  }
...
cluster_recursive = function(object, regress = F, regress_cc = NULL, do.magic = F)
{
  ...
  return(list(obj = object, mark = final_markers))
}
