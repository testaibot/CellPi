...
rec_final = cluster_recursive(curr_proc1, regress = F, regress_cc = NULL)
...
```{r}
object_list = split_all_ident(seur, do.magic = T)
merged_seur0 = seur_merge(object_list)
object_list1 = split_all_ident(merged_seur0, do.magic = T)
...
curr = merged_seur4
tab = table(get_orig_ident(curr), substr(sapply(get_ident(curr),toString), start = 1, stop = 5))
sort_diag(tab)
plot(sort(colMaxs(sort_diag(tab))))
sum(tab)
