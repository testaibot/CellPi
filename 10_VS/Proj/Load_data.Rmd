...
#curr_proc = full_preprocess(working_path  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\reads\\NanoString\\", counts_mtx  =  curr_assay)

# curr_assay = as.matrix(read.table("F:\\bioinf\\Sk\\SingleCellRNASeq\\reads\\cancer\\GSE6099_series_matrix.txt", row.names = 1, sep = "\t"))
# curr_assay[is.null(curr_assay)] = 0
# curr_proc = full_preprocess(working_path  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\reads\\NanoString\\", counts_mtx  =  curr_assay)
...
# new = load_sce(new)
# 
# curr_assay = m_bind(old, new, lA = "old", lB = "new")
# 
# curr_proc = full_preprocess(working_path  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\reads\\Svet\\", counts_mtx = curr_assay)
...
curr_proc0 = full_preprocess(working_path  =  "F:/bioinf/Sk/SingleCellRNASeq/outer/biase/", counts_mtx  =  curr_assay)

curr_assay = load_sce(readRDS(file  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\outer\\deng-reads.rds"))
curr_proc1 = full_preprocess(working_path  =  "F:/bioinf/Sk/SingleCellRNASeq/outer/deng-reads/", counts_mtx  =  curr_assay)



curr_assay = load_sce(readRDS(file  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\outer\\fan.rds"))
curr_proc2 = full_preprocess(working_path  =  "F:/bioinf/Sk/SingleCellRNASeq/outer/fan/", counts_mtx  =  curr_assay)

curr_assay = load_sce(readRDS(file  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\outer\\goolam.rds"))
curr_proc3 = full_preprocess(working_path  =  "F:/bioinf/Sk/SingleCellRNASeq/outer/goolam/", counts_mtx  =  curr_assay)

curr_assay = load_sce(readRDS(file  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\outer\\klein.rds"))
curr_proc4 = full_preprocess(working_path  =  "F:/bioinf/Sk/SingleCellRNASeq/outer/klein/", counts_mtx  =  curr_assay)

curr_assay = load_sce(readRDS(file  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\outer\\kolodziejczyk.rds"))
curr_proc5 = full_preprocess(working_path  =  "F:/bioinf/Sk/SingleCellRNASeq/outer/kolodziejczyk/", counts_mtx  =  curr_assay)
...
# 
# final_exprs = full_preprocess(working_path  =  "F:\\bioinf\\Sk\\SingleCellRNASeq\\reads\\klein\\mine", counts_mtx  =  d)

dat <- readRDS(url("http://pklab.med.harvard.edu/velocyto/mouseBM/cell.counts.matrices.rds"))
library(velocyto.R)

emat = full_preprocess(counts_mtx = dat$exon)
#nmat = full_preprocess(counts_mtx = dat$intron)
#smat = full_preprocess(counts_mtx = dat$spanning)
