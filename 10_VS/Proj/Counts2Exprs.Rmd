...
types2names <- function(types)
{
  ...
  get_annot_db <- function(gene_names, org = NULL)
  {
    if(is.null(org))
    {
  ...
  gene_annot_convert_to <- function(gene_names, new_encoding = "ENTREZID", org = NULL, annot = NULL)
  {
    if(is.null(annot)||is.null(org))
    {
      gene_annot <- detect_genome(gene_names)
    }
...
full_preprocess <- function(working_path = NULL, counts_mtx = NULL, cell_cycle = T, impute = F, markers = NULL, clean = FALSE)
{
  ...
  if(typeof(sce_cycled) != "character")
  {
    ...
    if(impute)
    {
      ...
      print("S")
      sce_saver <- lapply(sce_cycled, function(sce_set){return(2^exprs(simple_preprocessing(sce_set))-1)})
    }
  }
  else
  {
...
{
  A <- na.omit(rel2abs(saver_splited$x))
  B <- na.omit(rel2abs(saver_splited$y))
...
seur_norm_regress <- function(seur, regress = T, regress_cc_diff = T)
{
  ...
  if(regress)
  {
    ...
    seur1 <- tryCatch(CellCycleScoring(seur, G2M, S, set.ident = F), error = function(e) {"e"})
    
    if(typeof(seur1) != "character")
    {
      seur<-seur1
      if(regress_cc_diff)
      {
...
compute_pca <- function(seur)
{
  init_pc <- max(nrow(seur@raw.data)/120, 30)
  end_pc <- 10
  step <- (init_pc-end_pc)/10
  
  for(pc_use in unique(round(seq(init_pc, end_pc, -step))))
  {
    seur1 <- tryCatch(RunPCA(object = seur, pc.genes = rownames(seur@data), print.results = FALSE, pcs.print = NULL, do.print = F, pcs.compute = pc_use, rev.pca = T),
                      error = function(e) {"e"}, 
...
compute_tsne <- function(seur)
{
  init_plex <- max(ncol(seur@raw.data)/20, 30)
  end_plex <- 2
  step <- (init_plex-end_plex)/10
  
  for(perplex in unique(round(seq(init_plex, end_plex, -step))))
  {
    seur1 <- tryCatch(RunTSNE(object = seur, dims.use = 1:ncol(seur@dr$pca@cell.embeddings), dim.embed = 2, check_duplicates = FALSE, perplexity = perplex),error = function(e) {"e"})
    if(typeof(seur1) != "character")
    {
...
compute_clustering <- function(seur, mult = 1)
{
  k.param <- max(round(min(30, ncol(seur@raw.data)/3)),3)
  k.scale <- max(round(min(25, ncol(seur@raw.data)/4)),2)
  
  seur <- FindClusters(object = seur, algorithm = 3, k.param = k.param, k.scale = k.scale, modularity.fxn = 1, print.output = F, save.SNN = T, force.recalc = T)
  
  if(mult<2)
  {
    test_clustering_rez <- function(seur, rez)
    {
        s_out <- capture.output(FindClusters(object = seur, resolution = rez, k.param = k.param, k.scale = k.scale, algorithm = 3, modularity.fxn = 1, print.output = F, save.SNN = T, force.recalc = T))
    ...
    for(rez in seq(0.1, 100.1, 1))
    {
      if(test_clustering_rez(seur, rez)>0)
      {
        rez_curr <- rez
        break
      }
    ...
    seur <- FindClusters(object = seur, resolution = rez_curr, k.param = k.param, k.scale = k.scale, algorithm = 3, modularity.fxn = 1, print.output = F, save.SNN = T, force.recalc = T)
    seur1 <- ValidateClusters(seur, pc.use = 1:ncol(seur@dr$pca@cell.embeddings))
    if(length(table(get_ident(seur1))>1)>1)
    {
      print("Clustered...")
      return (seur1)
    }else{
      print("Clustered without correction...")
        if(length(table(get_ident(seur))>1)>1){
          return(seur)
        }
    }
    return("e")
    ...
    for(rez in seq(0.1, 0.5, 0.1))
    {
      seur <- FindClusters(object = seur, resolution = rez, k.param = k.param, k.scale = k.scale, algorithm = 3, modularity.fxn = 1, print.output = F, save.SNN = T, force.recalc = T)
      if(length(which(table(get_ident(seur))>1))>1){
...
seurat_analyse_mtx <- function(mtx, regress = T, regress_cc_diff = T, do.magic = T, do.cluster = T)
{
  if(do.magic)
  {
  ...
  seur <- CreateSeuratObject(raw.data = as.matrix(mtx))
  
  return(seurat_analyse(seur, regress = regress, regress_cc_diff = regress_cc_diff, do.cluster = do.cluster))
}

seurat_analyse <- function(seur, regress = T, regress_cc_diff = T, do.cluster = T)
{
  seur <- seur_norm_regress(seur, regress = regress, regress_cc_diff = regress_cc_diff)
  curr_mult <- get_copy_count(seur)
  
  seur1 <- compute_pca(seur)
  if(typeof(seur1) != "character")
  {
    seur <- seur1
  }else{
  ...
  seur <- compute_tsne(seur)
  
  if(do.cluster)
  {
    seur1 <- compute_clustering(seur, mult = curr_mult)
    if(typeof(seur1) != "character")
    {
      seur <- seur1
    }else{
...
seur_find_split <- function(seur, cluster = NULL, gran_shresh = 2, regress = T, regress_cc_diff = T, do.magic = F)
{
  curr_mult <- NULL
  if(is.null(cluster))
  {
  ...
  for(mult in seq(curr_mult, 10, 2))
  {
    ...
    seur_mult <- tryCatch(seurat_analyse_mtx(dub(raw_data, mult), regress = regress, regress_cc_diff = regress_cc_diff, do.magic = do.magic, do.cluster = T),error = function(e) {"e"})
    if(typeof(seur_mult) != "character")
    {
      if(length(which(table(get_ident(seur_mult))>gran_shresh))>1)
      {
        return(seur_mult)
      }
      if(length(table(get_ident(seur_mult)))==ncol(raw_data))
      {
        return(NULL)
      }
    }
...
merge_splits <- function(seur_obj_list)
{
  
  merged <- seur_obj_list[1]
  for(old_seur in seur_obj_list[2:length(seur_obj_list)])
  {
...
find_markers <- function(curr_seur, p_val = 0.05, test.use = "MAST", logfc.threshold = 0.5, only.pos = T, min.pct = 0.8, min.diff.pct = 0.5)
{
  ...
  {
    fm <- tryCatch(FindMarkers(object = curr_seur, ident.1 = ident,  test.use = test.use, logfc.threshold = logfc.threshold, only.pos = only.pos, min.pct = min.pct, min.diff.pct = min.diff.pct), error = function(e){"e"})
  ...
  for (i in 1:length(cluster.markers))
  {
    ...
    rownames(cluster.markers[i]$ident)
...
```{r}
plot_seur_3d <- function(seurat_datset, radius = 1)
{
  seurat_datset <- seur
  library(plyr)
  ...
  mfrow3d(1, 2)
  next3d()
  plot3d(x = rl[,1], y = rl[,2], z = rl[,3], col = mapvalues(curr_labels, from = array(labels_uniq$names), to = array(labels_uniq$colors)), type="s", radius=0.3)
  legend3d("topright", col = array(labels_uniq$colors), legend=labels_uniq$names, cex=3, pch = radius)
  next3d()
