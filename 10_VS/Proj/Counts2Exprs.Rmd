...
seurat_analyse <- function(mtx_or_seur, regress = T, regress_cc_diff = T, final_analyse = F)
{
  if(!final_analyse)
  {
    seur <- CreateSeuratObject(raw.data = as.matrix(mtx_or_seur))
...
seur_find_split <- function(seur, cluster = NULL, gran_shresh = 2, ...)
{
  curr_mult <- NULL
  if(is.null(cluster))
  {
    magic <- tryCatch(magic_impute(as.matrix(seur@raw.data)), error = function(e){"e"})
    ...
    magic <- tryCatch(as.matrix(seur@raw.data[, get_cell_names_by_ident(seur, cluster)]), error = function(e){"e"})
    if(typeof(magic) != "character")
    {
      print("magic")
      raw_data <- magic
  ...
  for(mult in seq(curr_mult, 20, 2))
  {
    ...
    seur_mult <- tryCatch(seurat_analyse(dub(raw_data, mult), ...),error = function(e) {"e"})
