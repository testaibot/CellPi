...
seurat_analyse <- function(mtx, regress = T, regress_cc_diff = F)
{
  ...
  {
    seur1 <- tryCatch(RunPCA(object = seur, pc.genes = rownames(seur@data), do.print = FALSE, pcs.print = NULL, genes.print = NULL, pcs.compute = pc_use), warning = function(w) {
      if("did not converge--results might be invlaid!; try increasing work or maxit" %in% w)
      {
        return(NULL)
      }
})
