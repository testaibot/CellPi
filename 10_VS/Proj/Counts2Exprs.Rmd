...
```{r}
  ...
  rel2abs <- function(mtx)
  {
    ...
    norm <- tryCatch(rel2abs_base(mtx), error = function(e){})
    if(is.null(norm))
    {
...
seurat_analyse <- function(mtx, regress = T, regress_cc_diff = F)
{
  ...
  {
    seur1 <- tryCatch(RunPCA(object = seur, pc.genes = rownames(seur@data), do.print = FALSE, pcs.print = NULL, genes.print = NULL, pcs.compute = pc_use, rev.pca = T),
