...
```{r}
  ...
  cell_cycle_decomposed_normalization <- function(sce)
  {
    ...
    saver_three_path_impute <- function(sce)
    {
      ...
      return(list(x = imputed_genes0, y = imputed_genes1, z = imputed_genes2))
    }
...
full_preprocess <- function(working_path = NULL, counts_mtx = NULL, cell_cycle = T, impute = F, markers = NULL, clean = FALSE)
{
  ...
  if(typeof(sce_cycled) != "character")
  {
    ...
    if(impute)
    {
      ...
      print("S")
      sce_saver <- lapply(sce_cycled, function(sce_set){return(2^simple_preprocessing(sce_set)-1)})
    }
  }
  else
  {
...
merge_saver_split <- function(saver_splited, lA = ".x", lB = ".y", lC = ".z")
{
  ...
  C <- na.omit(rel2abs(saver_splited$z))
...
{
  seur <- CreateSeuratObject(raw.data = my_data_init)
  ...
  for(pc_use in seq(100, 10,-10))
  {
    seur1 <- tryCatch(RunPCA(object = seur, pc.genes = rownames(seur@data), print.results = FALSE, pcs.print

 = NULL, do.print = F, pcs.compute = pc_use, rev.pca = T),
                      error = function(e) {"e"}, 
