...
```{r}
  ...
  rel2abs <- function(mtx)
  {
    ...
    norm <- tryCatch(rel2abs_base(mtx), error = function(e){print(e)})
    if(is.null(norm))
    {
      norm <- t(rel2abs_base(t(curr_proc)))
    }
    return(norm)
  }
...
seurat_analyse <- function(mtx, regress = T, regress_cc_diff = F)
{
 ...
 = NULL, do.print = F, pcs.compute = pc_use, rev.pca = T),
  ...
  {
    seur1 <- tryCatch(RunTSNE(object = seur, dims.use = 1:ncol(seur@dr$pca@cell.embeddings), dim.embed = 2, check_duplicates = FALSE, perplexity = perplex),error = function(e) {})
    if(!is.null(seur1))
    {
...
seur_find_multiplication <- function(seur, cluster = NULL, gran_shresh = 3, ...)
{
  ...
  for(mult in seq(curr_mult, 20, 2))
  {
    ...
    seur_mult <- tryCatch(seurat_analyse(dub(raw_data, mult), ...),error = function(e) {print(e)})
